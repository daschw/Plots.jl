...
function _precompile_()
    ...
    Base.precompile(Tuple{typeof(is_marker_supported),Shape})
    ...
    let fbody = try __lookup_kwbody__(which(ohlc, (Any,))) catch missing end
        if !ismissing(fbody)
            precompile(fbody, (Any,typeof(ohlc),Any,))
        end
    end
